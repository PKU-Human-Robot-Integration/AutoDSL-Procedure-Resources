...
class Compiler:
    def __init__(self) -> None:
        ...
        with open("data/semantic_baseline1_new.txt") as file:
            self.baseline1_prompt = file.read()
        with open("data/semantic_baseline4_1_new.txt") as file:
            self.baseline4_1_prompt = file.read()
        with open("data/semantic_baseline4_new.txt") as file:
            self.baseline4_2_prompt = file.read()
        pass

    def compile(self, subject, protocol, enum=0, old_protocol_log=None):
        '''
        ...
        for i, sentense in enumerate(sentense_list):
            ...
            statement_dsl.opcode = old_log["result_dsl"]["opcode"]
            entity_list = old_log["entity_list"]
            statement_dsl.slot = old_log["result_dsl"]["slot"]
            statement_dsl.emit = old_log["result_dsl"]["emit"]
            result["ours"].append(statement_dsl)

            ours_prompt = self.baseline1_prompt.replace("-*-*-*", str(list(datatype.type.keys()))).replace(
                "*-*-*-", statement_dsl.result("null"))
            for _ in range(5):
                json_output = self.__chatgpt_function(ours_prompt)
                format, statement_dsl_reprint = self.__check_json_output(json_output, datatype)
                if format:
                    result["dsl"].append(statement_dsl_reprint)
                    break

            kg = None
            for _ in range(5):
                baseline4_1_prompt = self.baseline4_1_prompt.replace("*-*-*-", "knowledge graph json").replace("*+*+*+", sentense) \
                                    .replace("-+-+-+", "json")
                json_output = self.__chatgpt_function(baseline4_1_prompt)
                format, dict = self.__check_json_output_pure(json_output)
                if format:
                    kg = dict
                    log_sentense["kg"] = dict
                    break
            if kg is None:
                log_sentense["kg"] = kg
            for _ in range(5):
                baseline4_2_prompt = self.baseline1_prompt.replace("-*-*-*", str(list(datatype.type.keys()))).replace(
                    "*-*-*-", str(kg))
            ...
            baseline2_prompt = self.baseline1_prompt.replace("-*-*-*", str(list(datatype.type.keys()))).replace(
                "*-*-*-", sentense)
            for _ in range(5):
            ...
            baseline3_prompt = self.baseline1_prompt.replace("-*-*-*", str(list(datatype.type.keys()))).replace(
                "*-*-*-", sentense)
            for _ in range(5):
            ...
            log_sentense["result_ours"] = statement_dsl.dict()
            log_sentense["result_dsl"] = statement_dsl_reprint.dict()
    ...
    def compile_1(self, subject, protocol, enum=0, old_protocol_log=None):
        ...
        for i, sentense in enumerate(sentense_list):
            ...
            for _ in range(5):
                baseline4_1_prompt = self.baseline4_1_prompt.replace("*-*-*-", "Python").replace("*+*+*+", sentense).replace("-+-+-+", "program")
            ...
            for _ in range(5):
                baseline5_1_prompt = self.baseline4_1_prompt.replace("*-*-*-", "Biocoder").replace("*+*+*+", sentense).replace("-+-+-+", "program")
            ...
            if isinstance(program, list):
                ...
                log_sentense["biocoder"] = program
            for _ in range(5):
                baseline5_2_prompt = self.baseline1_prompt.replace("-*-*-*", str(list(datatype.type.keys()))).replace(
                    "*-*-*-", program)
    ...
    def __similarity_opcode(self, subject, operation):
        ...
        for target_word in self.dsl[subject]:
            if target_word in self.word2vec_model:
                similarity = self.word2vec_model.similarity(operation.lower(),
                                                            self.lemmatizer.lemmatize(target_word.lower(), pos='v'))
    ...
    def __check_json_output(self, json_text, datatype):
        try:
            ...
            if "action" not in dict or "params" not in dict or "output" not in dict:
                return False, Statement()
            if (not isinstance(dict["action"], str)) or (not isinstance(dict["output"], str)) or (not isinstance(dict["params"], list)):
                return False, Statement()
            result = Statement()
            result.opcode = "" if dict["action"] is None else dict["action"]
            result.emit = "" if dict["output"] is None else dict["output"]
            params = [] if dict["params"] is None else dict["params"]
            for param in params:
                if len(param) != 2 or param[0] not in datatype.type:
                    return False, Statement()
            result.slot = params
    ...
    def __check_json_output_pure(self, json_text):
        try:
            dict = json.loads(json_text)
            return True, dict
        except:
